...
(function () {
    ...
    // ======================================================================
    // Save-to-Library: inject a "Save to Library" button on each generated
    // asset so it can be stored in the Asset Library (Phase 1 experimental).
    // ======================================================================
    (function initSaveToLibrary() {
        var OUTPUTS = {
            'product-output': 'product-understanding',
            'positioning-output': 'positioning',
            'story-output': 'story-architecture',
            'hooks-output': 'creative-hooks',
            'battlecard-output': 'battle-card',
            'winloss-output': 'win-loss',
            'gtm-output': 'pitch-deck',
            'segment-output': 'market-segment',
            'roadmap-output': 'roadmap-story'
        };

        function deriveTitle(container, type) {
            var h = container.querySelector('h3, h2, h4');
            var t = h ? h.textContent.trim() : '';
            if (!t) { t = type.replace(/-/g, ' ') + ' asset'; }
            return t.slice(0, 120);
        }

        function addSaveButton(container, type) {
            if (!container || !container.innerHTML.trim()) { return; }
            if (container.querySelector('.al-save-btn')) { return; }
            if (typeof AssetLibrary === 'undefined' || !AssetLibrary.save) { return; }
            var btn = document.createElement('button');
            btn.className = 'al-save-btn';
            btn.type = 'button';
            ...
            btn.addEventListener('click', function () {
                try {
                    AssetLibrary.save({
                        type: type,
                        title: deriveTitle(container, type),
        ...
        function scan() {
            Object.keys(OUTPUTS).forEach(function (id) {
                ...
                if (el && el.innerHTML.trim()) { addSaveButton(el, OUTPUTS[id]); }
            });
        }
        ...
        function observeAll() {
            Object.keys(OUTPUTS).forEach(function (id) {
                var el = document.getElementById(id);
                if (el) {
                    new MutationObserver(function () { addSaveButton(el, OUTPUTS[id]); })
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', function () { observeAll(); scan(); });
        } else {
            observeAll(); scan();
        }
    })();
